...
5. Run the following commands to ensure the integrity of your changes:
   ...
   - `bun test`: Execute the tests.
   - `bun circular`: Confirm there are no circular imports.
   - `bun lint`: Check for code style adherence (if you happen to encounter any errors during this process, you can use `bun lint-fix` to automatically fix some of these style issues).
   - `bun dtslint`: Run type-level tests.
   - `bun docgen`: Update the automatically generated documentation.
6. Create a changeset for your changes: before committing your changes, create a changeset to document the modifications. This helps in tracking and communicating the changes effectively. To create a changeset, run the following command: `bun changeset`. Always choose the `patch` option when prompted (please note that we are currently in pre-release mode).
