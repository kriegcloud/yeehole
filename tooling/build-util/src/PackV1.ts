...
import type { PackageJson } from "./PkgCtx";
import { PackageContext, PackageContextLive } from "./PkgCtx";
...
export const run = Effect.gen(function* (_) {
  ...
  const copySources = Effect.all([copyCjs, copyMjs, copyDts, copySrc], {
    concurrency: "inherit",
    discard: true,
  }).pipe(
  ...
  yield* _(
    Effect.all(
      [copyReadme, copyLicense, copyTsConfig, writePackageJson, copySources],
...
const replace = (content: string, path: string): string =>
  JSON.stringify(
    pipe(
      ...
      Array.map(([k, v]) =>
        k === "sources"
          ? ([k, Array.map(v as Array<string>, replaceString(path))] as const)
          : ([k, v] as const),
      ),
